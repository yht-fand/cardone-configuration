...
class r0004 implements java.io.Serializable {
    ...
    def func(input) {
		ApplicationContextHolder.getBean(ErrorInfoService.class).page(input)
    }
    ...
    def output(output) {
        ...
        for (def contentItem : output.getContent()) {
            ...
            data['errorInfoCode'] = contentItem['error_info_code']

            data['errorInfoId'] = contentItem['error_info_id']
            ...
            data['objectCode'] = contentItem['object_code']

            data['objectId'] = contentItem['object_id']

            data['objectTypeCode'] = contentItem['object_type_code']
            ...
            data['flagName'] = ApplicationContextHolder.getBean(DictionaryService.class).readOneByDictionaryTypeCodesCache(['dictionaryTypeCodes': 'errorInfoFlag,flag', 'dictionaryCode': contentItem['flag_code'], 'stateCode': '1', 'dataStateCode': '1',  'object_id': 'name'])

            data['stateName'] = ApplicationContextHolder.getBean(DictionaryService.class).readOneByDictionaryTypeCodesCache(['dictionaryTypeCodes': 'errorInfoState,state', 'dictionaryCode': contentItem['state_code'], 'stateCode': '1', 'dataStateCode': '1',  'object_id': 'name'])

            data['dataStateName'] = ApplicationContextHolder.getBean(DictionaryService.class).readOneByDictionaryTypeCodesCache(['dictionaryTypeCodes': 'errorInfoDataState,dataState', 'dictionaryCode': contentItem['data_state_code'], 'stateCode': '1', 'dataStateCode': '1',  'object_id': 'name'])
