...
class d0001 implements java.io.Serializable {
    def input(input) {
        ...
        for (def dictionaryItemId : dictionaryItemIds) {
            dictionaryItemIdList.add(["dictionaryItemId": dictionaryItemId, "dataStateCode": "0", "endDate": new Date()])
        }
    ...
    def func(input) {
        ['deleteCounts': ApplicationContextHolder.getBean(DictionaryItemService.class).updateListCache(input?.dictionaryItemIds)]
    }
